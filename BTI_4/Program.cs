...
namespace BTI_4
{
    class Program
    {
        ...
        private static void GetPosition(ref char[,] keySquare, char ch, ref int row, ref int
        col)
        {
            if (ch == 'J')
            ...
            private static char[] SameRow(ref char[,] keySquare, int row, int col1, int col2, int
            encipher)
        {
            return new char[] { keySquare[row, Mod((col1 + encipher), 5)], keySquare[row,
            Mod((col2 + encipher), 5)] };
        }
            private static char[] SameColumn(ref char[,] keySquare, int col, int row1, int row2,
            int encipher)
        {
            return new char[] { keySquare[Mod((row1 + encipher), 5), col], keySquare[Mod((row2
            + encipher), 5), col] };
        }
            private static char[] SameRowColumn(ref char[,] keySquare, int row, int col, int
            encipher)
        {
            return new char[] { keySquare[Mod((row + encipher), 5), Mod((col + encipher), 5)],
            keySquare[Mod((row + encipher), 5), Mod((col + encipher), 5)] };
        }
        private static char[] DifferentRowColumn(ref char[,] keySquare, int row1, int col1, int
        row2, int col2)
        {
        ...
        private static string Cipher(string input, string key, bool encipher)
        {
            ...
            for (int i = 0; i < tempInput.Length; i += 2)
            {
                ...
                {
                    retVal += new string(DifferentRowColumn(ref keySquare, row1, col1, row2,
                    col2));
                }
            }
        ...
        {
            while (true) { 
            Console.Write("Wprowadź słowo/tekst: ");
            string input = Console.ReadLine();
            Console.Write("Słowo klucz (bez powtarzających się liter): ");
            string key = Console.ReadLine();
            string enciphered = Encipher(input, key);
            string deciphered = Decipher(enciphered, key);
            Console.WriteLine("Zaszyfrowane słowo/tekst: {0}", enciphered);
            Console.WriteLine("Odszczyfrowane słowo/tekst: {0}", deciphered);
            Console.WriteLine("\n ---------------------- \n");
            }
        }
    }
